...
- ECや業務アプリのバックエンドは書けるけど、ゲーム案件の中身は見たことがない人
- ゲーム案件に入ることになって、何が待っているのか知りたい人
- 「ゲームサーバーって要はWebサーバーでしょ」と思っている人
...
ホーム画面、ガチャ、デッキ編成、クエスト、報酬受け取りなどを担当するのがAPIサーバーです。
ここは普通のHTTPのリクエスト/レスポンスで、ECの注文APIと形は同じ。

正直自分はEC系の実装を見たとき「なんてシンプルで簡潔なんだ。。。」と思いました。
同じWeb APIなのに何が違うのか、というところから見ていきます。
...
この4つが同時に存在することが、ゲーム案件のしんどさだと思っています。

# 1. 1回の操作が多くの機能に連鎖する

ECの注文APIなら、中心になるのは注文、注文明細、在庫です。
...
「アイテムは貰えたけどミッションは進んでいない」は、そのまま問い合わせになります。

ECの注文で「注文は作れたが在庫は減っていない」が許されないのと同じですが、ゲームは1回の操作が巻き込む機能の種類が多い。
...
通信のたびに全データを取り直すとレスポンスが膨らむので、サーバーは変化したものだけを返し、クライアントがローカルの状態へ適用します。
...
アプリを再起動すると直ってしまうので、報告が上がった頃には手元で再現しないという、一番やりにくい種類のバグになります。
...
このねじれが、差分同期、再送対策、不正対策をまとめて呼び込みます。
...
カードの攻撃力からスタミナ回復間隔まで、ゲームの数字はプランナーがスプレッドシートで調整します。
それがビルドされ、そのままサーバーとクライアントのロジックへ入ります。

不具合が出たときは、コードの問題、設定値の問題、クライアントとサーバーのバージョン違い、時刻境界の問題を切り分けるところから始まります。
コードを読む前にスプレッドシートを開く、ということも普通に起きます。
...
「釣り合った相手」と「早く始められること」のトレードオフを、時間軸で運用しなければなりません。
しかも、その緩め方がそのままゲームの手触りになります。

複数のサーバーが同じ待機列を見るなら、同じ人を2つの試合へ入れないよう、相手の選択と待機列からの削除を分割できない1つの操作として扱う必要があります。
その直後に部屋の作成が失敗した場合、待機列へ戻すのか、途中状態から復旧するのかも決めておかなければなりません。

対戦が始まると、状態の寿命はさらに長くなります。
試合の状態は数分から数十分にわたってサーバーのメモリへ残り、誰も操作していなくてもターン制限時間やゲーム内の時計で進みます。

同じ出来事でも、プレイヤーによって見せる内容が違います。
カードを引いた本人にはカード名を送り、相手には「1枚引いた」とだけ送る。
1つの状態から、見る人ごとに異なる情報を作る必要があります。

状態を持ち続けることは、インフラの運用にもそのまま影響します。
...
新しいバージョンを出すときは、古いサーバーへの新規割り当てだけ止め、進行中の試合がすべて終わるまで待ちます。
1試合10分なら、古いサーバーも最悪10分は残ります。

マスタデータを大きく切り替えるときは、旧ルールの試合と新ルールのクライアントを混ぜないため、全部止めるのが一番安いこともあります。

ゲームには「本日15:00〜17:00メンテナンス」が本当にあります。
ECで全画面を2時間止めたら大問題ですが、ゲームは石を配れば許してもらえるので、無停止へどこまで金と手間をかけるかという判断自体が変わってきます。

EC・業務APIの処理単位が1回のリクエストだとすれば、マッチングや対戦の処理単位は、数十秒から数十分続くセッションです。
この寿命の違いが、設計からデプロイまで全部に効いてきます。
...
難しいのは、これが同時に存在することです。

クエストクリアAPI 1本にも、報酬やミッションへの連鎖、差分レスポンス、再送対策、ルールのバージョン、不正チェック、時刻の固定が全部乗ってきます。
...
ゲームのサーバーは「APIを作る」より、**「APIを作るための土台を作る」の比重が大きい**。
...
2年ぶりに戻ってきて、面倒なのは何ひとつ変わっていなかったんですが、その面倒をどう潰すかが前より体系化されていたのは良かった。
